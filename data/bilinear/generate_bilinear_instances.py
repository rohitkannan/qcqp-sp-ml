...
bilinear_path = instances_path + "bilinear/"
json_path = instances_path + "json/"
mccormick_path = instances_path + "mccormick/"
os.makedirs(bilinear_path, exist_ok=True)
...
for inst in range(start,numInstances+start):
	output_file = bilinear_path + "qcqp_v" + str(numVariables) + "_b" + str(numBilinearTerms) + "_s" + str(round(constraint_sparsity*100)) + "_" + str(inst) + ".jl"
	mccormick_file = mccormick_path +  "qcqp_v" + str(numVariables) + "_b" + str(numBilinearTerms) + "_s" + str(round(constraint_sparsity*100)) + "_" + str(inst) + "_mccormick.jl"
	json_file = json_path + "qcqp_v" + str(numVariables) + "_b" + str(numBilinearTerms) + "_s" + str(round(constraint_sparsity*100)) + "_" + str(inst) + ".json"
...
# OBJECTIVE
	...
	f2.write("\n")
	f2.write("@objective(m, Min, sum(bilinear_obj_coeffs[i]*w[i] for i = 1:" + str(numBilinearTerms) + ") + sum(linear_obj_coeffs[i]*x[i] for i = 1:" + str(numVariables) + ")) \n\n\n")
...
# QUADRATIC CONSTRAINTS
	...
	for k in range(numQuadraticConstraints):
		...
		f2.write("@constraint(m, sum(bilinear_con_coeffs[" + str(count_quad_con) + ",i]*w[bilinear_con_coeff_indices[" + str(count_quad_con) + ",i]] for i = 1:" + str(numQuadNonZero) + ") + sum(linear_con_coeffs_1[" + str(count_lin_con) + ",i]*x[linear_con_coeff_indices_1[" + str(count_lin_con) + ",i]] for i = 1:" + str(numLinNonZero) + ") <= 1) \n\n\n")
...
# LINEAR CONSTRAINTS
	...
	for k in range(numLinearConstraints):
		...
		f2.write("@constraint(m, sum(linear_con_coeffs_2[" + str(count_lin_con - numQuadraticConstraints) + ",i]*x[linear_con_coeff_indices_2[" + str(count_lin_con - numQuadraticConstraints) + ",i]] for i = 1:" + str(numVariables) + ") == 1) \n\n\n")
