function amp_post_convhull(m::Optimizer; kwargs...)
    ...
    for k in keys(m.nonconvex_terms)
        ...
        if ((nl_type == :MULTILINEAR) || (nl_type == :BILINEAR)) && (m.nonconvex_terms[k][:convexified] == false)
            ...
            # ADDED
            λ, α, con1, con2, ml_indices, part_dim, sliced_indices = Alp.amp_convexify_multilinear(m, k, λ, α, con1, con2, sliced_indices, ml_indices, part_dim, d)
        elseif nl_type == :MONOMIAL && !m.nonconvex_terms[k][:convexified]
            λ, α, con3, con4, con5, con6, monomial_index, monomial_dim = Alp.amp_convexify_monomial(m, k, λ, α, con3, con4, con5, con6, monomial_index, monomial_dim, d)
    ...
    # ADDED return
    mip_pointers = Dict(:ext=>λ, :part=>α, :x_con=>con1, :aux_con=>con2, :indices=>ml_indices, :mon_x_con=>con3, :mon_aux_con=>con4, :mon_part_con_low=>con5, :mon_part_con_up=>con6, :mon_indices=>monomial_index, :dim=>part_dim, :sliced_indices=>sliced_indices)
    return mip_pointers
end

# function amp_convexify_multilinear(m::Optimizer, k::Any, λ::Dict, α::Dict, discretization::Dict)
# ADDED
function amp_convexify_multilinear(m::Optimizer, k::Any, λ::Dict, α::Dict, con1::Dict, con2::Dict, sliced_indices::Dict, ml_indices::Dict, dim::Dict, discretization::Dict)
...
=#
    # ADDED
    ml_indices[k], dim[k], extreme_point_cnt = Alp.amp_convhull_prepare(m, discretization, k)   # convert key to easy read mode
    λ = Alp.amp_convhull_λ(m, k, ml_indices[k], λ, extreme_point_cnt, dim[k])
    λ = Alp.populate_convhull_extreme_values(m, discretization, ml_indices[k], λ, dim[k], ones(Int,length(dim[k])))
    α = Alp.amp_convhull_α(m, ml_indices[k], α, dim[k], discretization)
    con1, con2, sliced_indices = Alp.amp_post_convhull_constrs(m, λ, α, con1, con2, sliced_indices, ml_indices[k], dim[k], extreme_point_cnt, discretization)

    return λ, α, con1, con2, ml_indices, dim, sliced_indices
end

function amp_convexify_monomial(m::Optimizer, k::Any, λ::Dict, α::Dict, con3::Dict, con4::Dict, con5::Dict, con6::Dict, monomial_index::Dict, monomial_dim::Dict, discretization::Dict)
...
=#
    # ADDED
    monomial_index[k], monomial_dim[k], extreme_point_cnt = Alp.amp_convhull_prepare(m, discretization, k, monomial=true)
    λ = Alp.amp_convhull_λ(m, k, monomial_index[k], λ, extreme_point_cnt, monomial_dim[k])
    λ = Alp.populate_convhull_extreme_values(m, discretization, monomial_index[k], λ, 2)
    α = Alp.amp_convhull_α(m, [monomial_index[k]], α, monomial_dim[k], discretization)
    con3, con4, con5, con6 = Alp.amp_post_convhull_constrs(m, λ, α, con3, con4, con5, con6, monomial_index[k], monomial_dim[k], discretization)

    return λ, α, con3, con4, con5, con6, monomial_index, monomial_dim
end
...
# ADDED
function amp_post_convhull_constrs(m::Optimizer, λ::Dict, α::Dict, con1::Dict, con2::Dict, sliced_indices::Dict, indices::Any, dim::Tuple, ext_cnt::Int, d::Dict)
#=
...
=#
    ...
    # Adding λ constraints
    JuMP.@constraint(m.model_mip, sum(λ[indices][:vars]) == 1)
    con2[indices] = JuMP.@constraint(m.model_mip, _index_to_variable_ref(m.model_mip, λ[indices][:lifted_var_idx]) == dot(λ[indices][:vars], reshape(λ[indices][:vals], ext_cnt)))
    ...
    for (cnt, i) in enumerate(indices)
        ...
        sliced_indices_tmp = [collect_indices(λ[indices][:indices], cnt, [k], dim) for k in 1:l_cnt] # Add x = f(λ) for convex representation of x value
        sliced_indices[(indices, i)] = sliced_indices_tmp
        con1[(indices, i)] = JuMP.@constraint(m.model_mip, _index_to_variable_ref(m.model_mip, i) == sum(dot(repeat([d[i][k]],length(sliced_indices_tmp[k])), λ[indices][:vars][sliced_indices_tmp[k]]) for k in 1:l_cnt))
    end

    return con1, con2, sliced_indices
end
...
# ADDED
function amp_post_convhull_constrs(m::Optimizer, λ::Dict, α::Dict, con3::Dict, con4::Dict, con5::Dict, con6::Dict, monomial_idx::Int, dim::Tuple, discretization::Dict)
    ...
    # ADDED
    con4[monomial_idx] = JuMP.@constraint(m.model_mip, _index_to_variable_ref(m.model_mip, λ[monomial_idx][:lifted_var_idx]) <= dot(λ[monomial_idx][:vars], λ[monomial_idx][:vals]))
    # ADDED
    # should we outer-approximate quadratics in the relaxation?
    if Alp.get_option(m, :outer_approximate_quadratics)
        numLinPoints = 20
        linPoints = [range(discretization[monomial_idx][1],discretization[monomial_idx][end],length=numLinPoints);]
        for pt in linPoints
            JuMP.@constraint(m.model_mip, _index_to_variable_ref(m.model_mip, λ[monomial_idx][:lifted_var_idx]) >= 2*pt*(_index_to_variable_ref(m.model_mip, monomial_idx)) - pt^2)
        end
    else
    ...
    if Alp.get_option(m, :convhull_ebd) && partition_cnt > 2
        ...
        # ADDED
        con5[monomial_idx] = JuMP.@constraint(m.model_mip, _index_to_variable_ref(m.model_mip, monomial_idx) >= sum(α[monomial_idx][j]*discretization[monomial_idx][j] for j in 1:lambda_cnt-1))
        con6[monomial_idx] = JuMP.@constraint(m.model_mip, _index_to_variable_ref(m.model_mip, monomial_idx) <= sum(α[monomial_idx][j-1]*discretization[monomial_idx][j] for j in 2:lambda_cnt))
    end
    ...
    # ADDED
    con3[monomial_idx] = JuMP.@constraint(m.model_mip, _index_to_variable_ref(m.model_mip, monomial_idx) == dot(λ[monomial_idx][:vars], discretization[monomial_idx]))

    # return
    # ADDED
    return con3, con4, con5, con6
end
