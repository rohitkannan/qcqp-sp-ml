...
for inst in range(numInstances):
	features_file = base_dir + 'results/bilinear/n=20/ml_features/d=2/qcqp_v20_b100_s100_' + str(inst+1) + '_features.json'
...
for fold in range(1):
	...
	for part_num in range(part_dim):
		...
		part_pred_train = regressor.predict(training_features)
		mse_val_train = mean_squared_error(training_part_points[:, part_num], part_pred_train)
		mae_val_train = mean_absolute_error(training_part_points[:, part_num], part_pred_train)
		print("part_num: ", part_num, "  train mse: ", mse_val_train, "  train mae: ", mae_val_train, 3)
	...
	for i in rand_perm[testing_set]:
		...
		part_file = 'ab_predicted_part_points/qcqp_v20_b100_s100_' + str(i+1) + '_ab_pred.json'
