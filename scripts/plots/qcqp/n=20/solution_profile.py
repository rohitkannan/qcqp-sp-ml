...
#    print("scaled_metric: " + str(scaled_metric))
#    print("scaled_metric_dist: " + str(scaled_metric_dist))
    ...
    for m in range(num_methods):
        if m == 1:
            ...
            plt.plot(time_values,sorted_metric_dist[m],label=methods[m],linewidth=4.0,marker='s',markersize=6,linestyle='')
        else:
            plt.plot(time_values,sorted_metric_dist[m],label=methods[m],linewidth=4.0)
    plt.legend()
    plt.xscale('log')
    # plt.ylabel('Percentage of instances solved within time T')
    plt.ylabel('% instances solved within time T')
    plt.xlabel('Time T (seconds)')
    plt.title(title_name)
    plt.grid(lw=0.1)
    # plt.xticks([0.1, 0.2, 0.5, 1.0, 2.0, 3.0],['0.1','0.2','0.5','1','2','3'])
    # plt.xticks([0.1, 0.2, 0.5, 1.0, 2.0],['0.1','0.2','0.5','1','2'])
    plt.xticks([0.5, 2, 5, 20, 50, 200, 500],['0.5','2','5','20','50','200',''])
    # plt.xticks([2, 5, 20, 50, 200, 500, 2000],['2','5','20','50','200','500','2000'])

    # plt.subplots_adjust(left=0.10,right=0.99,bottom=0.12,top=0.94)
    plt.subplots_adjust(left=0.14,right=0.99,bottom=0.12,top=0.94)
